...
function slashDelimit() {
    return Array.prototype.slice.call(arguments).filter(function(str) {
        return str;
    }).join('/');
}
...
export default function() {
    ...
    if (!page || hiddenIds[page.pageId] === true) {
        return;
    }
        ...
        function(resp) {
            var collections = (resp.collections || [])
                .filter(function(collection) {
                    return _.isArray(collection.content) && collection.content.length;
                })
                ...
                articles = _.flatten([
                    collections.filter(function(c) {
                        return c.href === 'global';
                    }).map(function(c) {
                        return c.content;
                    }),
                    collections.filter(function(c) {
                        return pageMatchers[c.href];
                    }).map(function(c) {
                        return c.content;
                    })
                ]),
            ...
            if (alerts.length) {
                ...
                setTimeout(function() {
                    ...
                    s.eVar36 = message;
                    s.eVar72 = _.map(alerts, function(article) {
