...
var resizeTimeout,
    ...
    defineSlot = function($adSlot) {
        var slotTarget = $adSlot.data('slot-target') || $adSlot.data('name'),
            ...
            adUnit = adUnitOverride ? ['/', config.page.dfpAccountId, '/', adUnitOverride].join('') : config.page.adUnit,
            id = $adSlot.attr('id'),
    ...
    parseAd = function(event) {
        ...
        if (event.isEmpty) {
            ...
            if (($slot.hasClass('ad-slot--top-banner-ad') && size === '88,70') || ($slot.hasClass('ad-slot--commercial-component') && size === '88,88')) {
