...
import LiveFilter from 'common/modules/live/filter';
...
function createScrollTransitions() {
    ...
    function unselectOnScroll() {
        bean.off(curBinding);
        curBinding = mediator.once('window:scroll', function() {
            unselect();
        });
    }
    ...
    if (timeline) {
        bean.on(timeline, 'click', '.timeline__link', function(e) {
            ...
            url.pushUrl({
                blockId: eventId
            }, title, window.location.pathname + '#' + eventId, true);
            e.stop();
        });
    }
}
...
modules = {
    ...
    createAutoUpdate: function() {
        ...
        if (config.page.isLive) {
            ...
            var timerDelay = detect.isBreakpoint({
                min: 'desktop'
            }) ? 5000 : 60000;
